...
void SkPathShapeStroker::quadTo(const SkPoint& pt1, const SkPoint& pt2) {
    bool degenerateAB = SkPath::IsLineDegenerate(fPrevPt, pt1, false);
    bool degenerateBC = SkPath::IsLineDegenerate(pt1, pt2, false);
...

    {
        ...
        if (SkChopQuadAtMaxCurvature(pts, tmp) == 2) {
            ...
            if (normals_too_pinchy(unitAB, unitBC)) {
                ...
                SkRect circle;
                circle.set (tmp[2].fX - fRadius, tmp[2].fY - fRadius, tmp[2].fX + fRadius, tmp[2].fY + fRadius);
                SkScalar cx = circle.centerX();
                SkScalar cy = circle.centerY();
                SkScalar rx = SkScalarHalf(circle.width());
                SkScalar ry = SkScalarHalf(circle.height());
                ...
                SkScalar L = circle.fLeft;
                SkScalar T = circle.fTop;
                SkScalar R = circle.fRight;
                SkScalar B = circle.fBottom;

                fJoinsAndCaps.incReserve(17);
                fJoinsAndCaps.moveTo(R, cy);

                fJoinsAndCaps.quadTo(R, cy + sy, cx + mx, cy + my);
                fJoinsAndCaps.quadTo(cx + sx, B, cx, B);
                fJoinsAndCaps.quadTo(cx - sx, B, cx - mx, cy + my);
                fJoinsAndCaps.quadTo(L, cx + sy, L, cy);
                fJoinsAndCaps.quadTo(L, cy - sy, cx - mx, cy - my);
                fJoinsAndCaps.quadTo(cx - sx, T, cx, T);
                fJoinsAndCaps.quadTo(cx + sx, T, cx + mx, cy - my);
                fJoinsAndCaps.quadTo(R, cy - sy, R, cy);
                fJoinsAndCaps.close();
...
void SkPathShapeStroker::cubicTo(const SkPoint& pt1, const SkPoint& pt2,
    ...
    bool degenerateAB = SkPath::IsLineDegenerate(fPrevPt, pt1, false);
    bool degenerateBC = SkPath::IsLineDegenerate(pt1, pt2, false);
    bool degenerateCD = SkPath::IsLineDegenerate(pt2, pt3, false);
